...
export default class Game extends Component {
  ...
  handleMouseClick(row, col, player) {
    ...
    if (!this.state.gameRunning || !this.state.samePC) {
      if (!this.state.gameRunning || this.user !== curr_player) {
        return;
      }
    }
    ...
    // If piece to move not slected, initial_click will be [-1,-1]
      ...
      var new_grid;
      [isValidMove, new_grid, piece_killed] = checkIfValidMove(
        this.state.initial_click,
        row,
        col,
        this.state.grid
      );
      if (isValidMove) {
        ...
        this.setState({ new_grid }, () => {
          if (!this.state.samePC) {
            this.socket.emit("move made", this.state);
          }
        });
    ...
    if (piece_killed === "king") {
      this.setState({ curr_player: 0, winner_color: this.user }); // Donno why i need to add here as well
      this.endGame(1);
    }
  }
  ...
  gameResult(winner) {
    ...
    this.me_ready = 0;
    this.opp_ready = 0;
    console.log("WInner is : ", this.state.winner_color, winner);
  }
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-sm-9" class="col-md-9" class="col-lg-9">
              <button
                onMouseEnter={() => this.showHighlighter(1)}
                onMouseLeave={() => this.showHighlighter(0)}
                type="button"
                class="btn btn-info btn-sm ml-5 mr-5 mt-2"
                onClick={() => this.rotate(0, 0)}
              >
                Rotate-A
              </button>
              <button
                onMouseEnter={() => this.showHighlighter(2)}
                onMouseLeave={() => this.showHighlighter(0)}
                type="button"
                class="btn btn-info btn-sm ml-5 mr-5 mt-2"
                onClick={() => this.rotate(0, 1)}
              >
                Rotate-B
              </button>
              <button
                onMouseEnter={() => this.showHighlighter(3)}
                onMouseLeave={() => this.showHighlighter(0)}
                type="button"
                class="btn btn-info btn-sm ml-5 mr-5 mt-2"
                onClick={() => this.rotate(0, 2)}
              >
                Rotate-C
              </button>
              <div className="grid">
                <>
                  <div class="highlightGrid">
                    <div
                      class={"highlighter board" + this.state.highlight}
                    ></div>
                  </div>
                  {grid.map((row, rowIdx) => {
                    return (
                          ...
                          return (
                            <Node
                              ...
                              onClick={(row, col, player) =>
                                this.handleMouseClick(row, col, player)
                              }
                              player={player}
                              piece={piece}
                            ></Node>
              ...
              <button
                onMouseEnter={() => this.showHighlighter(4)}
                onMouseLeave={() => this.showHighlighter(0)}
                type="button"
                class="btn btn-info btn-sm ml-5 mr-5"
                onClick={() => this.rotate(1, 0)}
              >
                Rotate-D
              </button>
              <button
                onMouseEnter={() => this.showHighlighter(5)}
                onMouseLeave={() => this.showHighlighter(0)}
                type="button"
                class="btn btn-info btn-sm ml-5 mr-5"
                onClick={() => this.rotate(1, 1)}
              >
                Rotate-E
              </button>
              <button
                onMouseEnter={() => this.showHighlighter(6)}
                onMouseLeave={() => this.showHighlighter(0)}
                type="button"
                class="btn btn-info btn-sm ml-5 mr-5"
                onClick={() => this.rotate(1, 2)}
              >
                Rotate-F
              </button>
            </div>
            {this.state.gameRunning ? (
              ...
              <OutsideGame
                startSamePC={this.startSamePC}
                joinRoom={(roomId) => this.joinRoom(roomId)}
                startNewRoom={(timeLimitEntered) =>
                  this.startNewRoom(timeLimitEntered)
                }
              />
            )}
          </div>
...
const getInitialGrid = () => {
  ...
  for (let row = 0; row < 10; row++) {
    ...
    for (let col = 0; col < 15; col++) {
      ...
      currentRow.push(
        createNode(col, row, isLeft, isRight, isBottom, isTop, player, piece)
      );
    }
